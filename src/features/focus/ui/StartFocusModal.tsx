...
const MODES: Array<{ mode: TimerMode; label: string; detail: string }> = [
  ...
  { mode: 'custom', label: 'Custom', detail: '1–180 min' },
...
export function StartFocusModal() {
  ...
  return <PixelModal open={open} onClose={close} title="START A FOCUS BLOCK">
    ...
    {mode === 'custom' ? <label className="custom-minutes-row"><span>MINUTES</span><input type="number" min={1} max={180} value={customMinutes} onChange={e => setCustomMinutes(Math.max(1, Math.min(180, Number(e.target.value) || 1)))} /></label> : null}
